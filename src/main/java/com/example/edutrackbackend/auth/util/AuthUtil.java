package com.example.edutrackbackend.auth.util;
...
public class AuthUtil {
    ...
    public static CustomUserDetails getCurrentUser() {
        ...
        if (auth == null || !auth.isAuthenticated()) throw new RuntimeException("Unauthorized request");

        Object principal = auth.getPrincipal();

        if (principal instanceof CustomUserDetails user) { return user; }
    ...
    public static Long getUserId() { return getCurrentUser().getUserId(); }

    // Returns the email (username) of the currently authenticated user
    public static String getEmail() { return getCurrentUser().getUsername();}
